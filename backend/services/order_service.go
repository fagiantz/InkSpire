...
func (s *OrderService) GetActiveOrders() ([]models.Order, error) {
	...
	if err := s.db.Preload("OrderItems").Where("status IN ?", []string{"unpaid", "process", "paid"}).Order("order_date desc").Find(&orders).Error; err != nil {
		return nil, err
	}
	return orders, nil
}
...
func (s *OrderService) GetActiveOrdersByUserID(userID uint) ([]models.Order, error) {
	...
	var orders []models.Order
	if err := s.db.Preload("OrderItems").Where("email_pembeli = ? AND status IN ?", user.Email, []string{"unpaid", "process", "paid"}).Order("order_date desc").Find(&orders).Error; err != nil {
		return nil, err
	}
	return orders, nil
}
...
func (s *OrderService) GetOrderById(orderID uint) (*models.Order, error) {
	...
	if err := s.db.Preload("OrderItems").First(&order, orderID).Error; err != nil {
		return nil, err
	}
	return &order, nil
}
